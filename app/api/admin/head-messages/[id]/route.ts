...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (!id) {
      return NextResponse.json({ error: 'Missing head message ID' }, { status: 400 })
    }
    ...
    console.log('Received PUT request with image_url:', image_url)

    const service = createServiceClient(SUPABASE_URL, SERVICE_ROLE_KEY)

    const { data, error } = await service
      .from('head_messages')
      .update({ title, name, message, is_active, image_url, updated_at: new Date().toISOString() })
      .eq('id', id)
      .select()
      .single()
    ...
    console.error('PUT /api/admin/head-messages/[id] error:', err)
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const service = createServiceClient(SUPABASE_URL, SERVICE_ROLE_KEY)
    const { error } = await service.from('head_messages').delete().eq('id', id)
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    return NextResponse.json({ success: true }, { status: 200 })
  } catch (err: any) {
    console.error('DELETE /api/admin/head-messages/[id] error:', err)
    const isProd = process.env.NODE_ENV === 'production'
    return NextResponse.json(
      { error: err?.message || 'Unknown error', stack: isProd ? undefined : err?.stack },
      { status: 500 }
